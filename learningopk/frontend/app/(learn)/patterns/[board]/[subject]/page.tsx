...
export default async function PatternPage({ params, searchParams }: PatternPageProps) {
  ...
  const subjectEntry =
    subjectsList?.subjects.find(
      (item) =>
        item.boardSlug === route.board &&
        item.slug === route.subject &&
        (!query.grade || item.classSlug === query.grade || item.grade === query.grade)
    ) ?? null;

  const grade = query.grade ?? subjectEntry?.classSlug ?? subjectEntry?.grade ?? "9";
  const payload = await getPatternAnalysis({
    board: route.board,
    grade,
  ...
  return (
    <AppShell
      session={session}
      currentPath={`/patterns/${payload.board.slug}/${payload.subject.slug}`}
      ...
      <div className="space-y-6">
        <PageHeader
          ...
          breadcrumbs={[
            ...
            { label: "Subjects", href: "/subjects" },
            {
              ...
              href: `/${payload.board.slug}/${grade}/${payload.subject.slug}`,
            },
            { label: "Exam Patterns" },
          ]}
        />
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <h1 className="text-2xl font-semibold text-text-primary">
                {payload.subject.name} topic weightage
              </h1>
              <p className="mt-2 text-sm text-text-secondary">
                Review 5-year topic trends and focus your revision on the most recurring board-exam
                chapters.
              </p>
            </div>
            <Link
              href={`/${payload.board.slug}/${grade}/${payload.subject.slug}`}
              className="rounded-lg border border-border-default px-4 py-2 text-sm font-medium text-text-primary transition hover:border-accent-primary/40"
            >
              Back to subject
            </Link>
          </div>
        ...
        {payload.recommendation ? (
          <section className="rounded-2xl border border-emerald-500/25 bg-emerald-500/10 p-5 text-sm text-text-primary">
            Focus {payload.recommendation.focusPercent}% of your time on these{" "}
            {payload.recommendation.chapterCount} high-weight chapters:{" "}
            {payload.recommendation.chapters.join(", ")}
          </section>
        ) : null}
              ...
              <div className="mb-4 flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-xs text-text-secondary">
                    Appeared in {chapter.weightagePercentage}% of recent exams • Avg{" "}
                    {Math.round(chapter.avgMarks)} marks
                  </p>
                </div>
                <Badge variant={chapter.weightagePercentage >= 60 ? "warning" : "outline"}>
