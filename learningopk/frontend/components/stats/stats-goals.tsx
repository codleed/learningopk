      ...
      {goals.map((goal) => (
        <DashboardCard key={goal.label} className="p-4">
          <div className="flex items-start justify-between gap-3">
            ...
            <span className="text-xs font-semibold text-[var(--primary)]">{goal.valueLabel}</span>
          </div>
          <div className="mt-3 h-2 w-full overflow-hidden rounded-full bg-muted">
            <span className="block h-full rounded-full bg-[var(--primary)]" style={{ width: `${goal.progressPercent}%` }} />
          </div>
