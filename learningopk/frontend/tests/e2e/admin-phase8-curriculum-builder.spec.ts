...
test("admin content renders curriculum builder controls", async ({ page }) => {
  ...
  await expect(
    page.locator("textarea[data-testid='curriculum-chapter-summary-input']")
  ).toBeVisible();
...
test("chapter summary preview renders markdown, images, and math", async ({ page }) => {
  ...
  await page
    ...
    .fill("$$x = 8 \\quad \\text{or} \\quad x = 3$$");
  await page.getByTestId("curriculum-chapter-summary-preview-toggle").click();

  await expect(
    page.locator("[data-testid='curriculum-chapter-summary-preview'] .katex-display")
  ).toHaveCount(1);
  await expect(
    page.locator("[data-testid='curriculum-chapter-summary-preview']")
  ).not.toContainText("$$x = 8");
});
...
test("chapter summary preview preserves heading hierarchy for h1 h2 h3", async ({ page }) => {
  ...
  const sizes = await preview.evaluate(() => {
    const h1El = Array.from(
      document.querySelectorAll("[data-testid='curriculum-chapter-summary-preview'] h1")
    ).find((node) => node.textContent?.includes("Heading 1")) as HTMLElement | undefined;
    const h2El = Array.from(
      document.querySelectorAll("[data-testid='curriculum-chapter-summary-preview'] h2")
    ).find((node) => node.textContent?.includes("Heading 2")) as HTMLElement | undefined;
    const h3El = Array.from(
      document.querySelectorAll("[data-testid='curriculum-chapter-summary-preview'] h3")
    ).find((node) => node.textContent?.includes("Heading 3")) as HTMLElement | undefined;
...
test("chapter summary preview renders core markdown formatting primitives", async ({ page }) => {
  ...
  await expect(preview.locator("pre code.language-javascript")).toContainText(
    "console.log(gemini);"
  );
...
test("student chapter summary screen renders markdown content", async ({ page }) => {
  ...
  await page
    .getByTestId("curriculum-chapter-summary-input")
    .fill(
      '![PreviewDiagramMd](https://example.com/momentum.png "width=220 height=140")\n\n$$x = 8 \\quad \\text{or} \\quad x = 3$$'
    );
  await page.getByTestId("curriculum-chapter-submit").click();

  const curriculumResponse = await page.request.get(
    "http://localhost:3001/api/admin/content/curriculum"
  );
  ...
  const publishResponse = await page.request.post(
    `http://localhost:3001/api/admin/content/chapters/${chapter?.id}/publish`,
    {
  ...
  const image = page.locator(
    "[data-testid='chapter-summary-markdown'] img[alt='PreviewDiagramMd']"
  );
  await expect(image).toHaveCount(1);
  await expect(image).toHaveCSS("width", "220px");
  await expect(image).toHaveCSS("height", "140px");
  await expect(page.locator("[data-testid='chapter-summary-markdown'] .katex-display")).toHaveCount(
    1
  );
  await expect(page.locator("[data-testid='chapter-summary-markdown']")).not.toContainText(
    "$$x = 8"
  );
});
...
test("admin summary editor saves markdown for selected chapter", async ({ page }) => {
  ...
  await page
    .getByTestId("curriculum-chapter-summary-input")
    .fill("Draft summary from create form.");
  await page.getByTestId("curriculum-chapter-submit").click();
  ...
  const publishResponse = await page.request.post(
    `http://localhost:3001/api/admin/content/chapters/${chapterId}/publish`,
    {
      data: { isPublished: true },
    }
  );
...
test("admin add chapter imports markdown file into summary draft before submit", async ({
  page,
}) => {
  await loginAsSeededAdmin(page);
  await page.goto("/admin/content");
  ...
  await expect(page.getByTestId("curriculum-chapter-summary-preview")).toContainText(
...
test("admin can create board class subject chapter using curriculum builder", async ({ page }) => {
  ...
  await page
    .getByTestId("curriculum-chapter-summary-input")
    .fill("Chapter summary for e2e coverage.");
  await page.getByTestId("curriculum-chapter-submit").click();
...
test("admin can add exercise from exercise tab with physics numerical option", async ({ page }) => {
  ...
  await expect(page.getByTestId("curriculum-exercise-type-select")).toContainText(
    "Comprehension Questions Long Questions"
  );
  await expect(page.getByTestId("curriculum-exercise-type-select")).toContainText(
    "Numerical Problems"
  );
  ...
  await page.goto(
    `/${toSlug(boardName)}/${toSlug(className)}/${toSlug(subjectName)}/${toSlug(chapterTitle)}?tab=exercises`
  );
  await expect(
    page.getByText("A body has mass 2kg and acceleration 3m/s^2. Find force.")
  ).toBeVisible();
});
