...
export function useGamification() {
  ...
  const markSummaryRead = useCallback(
    (chapterId: string) => {
  ...
  const markSubpartRead = useCallback(
    (chapterId: string, subpartId: number, totalSubparts: number) => {
  ...
  const markExerciseComplete = useCallback(
    (chapterId: string, exerciseId: number, difficulty: "easy" | "medium" | "hard") => {
      const progress = getChapterProgress(chapterId);
  ...
  const markFlashcardReviewed = useCallback(
    (chapterId: string, cardId: string, status: CardStatus) => {
      const progress = getChapterProgress(chapterId);
      const xpAmount =
        status === "known" ? XP_REWARDS.FLASHCARD_KNOWN : XP_REWARDS.FLASHCARD_REVIEW;
  ...
  const completeQuiz = useCallback(
    (chapterId: string, score: number, percentage: number) => {
      const progress = getChapterProgress(chapterId);
