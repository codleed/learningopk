...
import { Award, Flame, Globe, Sparkles, Target, TrendingDown, TrendingUp, Trophy, Users } from "lucide-react";

import { PageHeader } from "@/components/common/page-header";
import { AppShell } from "@/components/foundation/app-shell";
import { Avatar, Badge, Card, CardBody, CardDescription, CardHeader, CardTitle, EmptyState, TabContent, TabList, Tabs, TabTrigger } from "@/components/ui";
import type { LeaderboardMetric, LeaderboardResponse, LeaderboardScope } from "@/lib/leaderboard-api";
...
export function LeaderboardPageClient({ session, leaderboards, metric, error }: LeaderboardPageClientProps) {
    ...
    >
      <div className="space-y-6">
        ...
        <div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
          <Card variant="gradient" style={{ ["--card-gradient" as string]: "linear-gradient(135deg, rgba(56,189,248,0.45), rgba(99,102,241,0.25), rgba(251,191,36,0.22))" }}>
            <CardBody className="flex flex-col gap-5 px-6 py-6 sm:flex-row sm:items-end sm:justify-between">
              <div className="space-y-3">
                <Badge variant="outline" className="w-fit gap-2 border-white/20 bg-white/10 text-text-primary backdrop-blur">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-2xl font-semibold text-text-primary">{currentMetric.label} standings</p>
                      <p className="text-sm text-text-secondary">See how you stack up across active learners.</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid gap-3 sm:min-w-[18rem] sm:grid-cols-2">
                <MetricPill icon={Globe} label="Global students" value={leaderboards.global?.currentUser.totalStudents ?? 0} />
                <MetricPill icon={Users} label="Your rank" value={activeLeaderboard?.currentUser.rank ?? 0} />
              </div>
            </CardBody>
          </Card>
            ...
            <CardBody className="space-y-3 pt-0">
              <div className="rounded-2xl border border-border-default bg-bg-subtle/60 p-4">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-xs uppercase tracking-[0.16em] text-text-muted">Visibility</p>
                    <p className="mt-1 text-sm font-medium text-text-primary">
                      {activeLeaderboard?.currentUser.leaderboardPublic ? "Public leaderboard enabled" : "Private leaderboard enabled"}
                    </p>
                  </div>
                  <Badge variant={activeLeaderboard?.currentUser.leaderboardPublic ? "success" : "warning"}>
              ...
              {activeLeaderboard?.currentUser.badge ? (
                <Badge className={cn("w-fit border px-3 py-1 text-xs uppercase tracking-[0.18em]", badgeStyles[activeLeaderboard.currentUser.badge])}>
                  <Award className="h-3.5 w-3.5" />
                  Top 100 {activeLeaderboard.currentUser.badge}
                </Badge>
              ) : (
                <p className="text-sm text-text-secondary">Break into the global top 100 in XP to unlock a profile badge.</p>
              )}
            </CardBody>
          </Card>
        </div>

        <Tabs value={activeScope} onValueChange={(value) => setActiveScope(value as LeaderboardScope)}>
          <TabList variant="pills" className="w-fit flex-wrap">
            {scopeTabs.map((tab) => (
              <TabTrigger key={tab.value} value={tab.value} variant="pills" layoutId="leaderboard-scope-tab">
                {tab.label}
              </TabTrigger>
            ))}
          </TabList>
                  ...
                  <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.22, ease: "easeOut" }}>
                    <Card>
                      <CardHeader>
                        ...
                        <CardDescription>
                          Ranked by {currentMetric.label.toLowerCase()} with weekly momentum and badge highlights.
                        </CardDescription>
                      </CardHeader>
                      <CardBody className="space-y-3 pt-0">
                        {leaderboard.entries.map((entry) => (
                          ...
                          >
                            <div className="flex items-center gap-3">
                              ...
                              {entry.badge ? (
                                <Badge className={cn("border px-2 py-0.5 text-[10px] uppercase tracking-[0.16em]", badgeStyles[entry.badge])}>
                                  {entry.badge}
                                </Badge>
                              ) : null}
                            </div>
                            ...
                            <div className="flex min-w-0 items-center gap-3">
                              ...
                              <div className="min-w-0">
                                <div className="flex items-center gap-2">
                                  ...
                                  {entry.isCurrentUser ? <Badge variant="primary" size="sm">You</Badge> : null}
                                </div>
                                <p className="text-xs text-text-secondary">Level {entry.level} • {entry.xp.toLocaleString()} XP</p>
                              </div>
                            </div>

                            <StatColumn label="Streak" value={`${entry.streak} days`} icon={Flame} />
                            <StatColumn label="Quizzes" value={entry.quizzes.toLocaleString()} icon={Target} />
                            <StatColumn
                              label="Weekly change"
...
function MetricPill({ icon: Icon, label, value }: { icon: typeof Globe; label: string; value: number }) {
  return (
