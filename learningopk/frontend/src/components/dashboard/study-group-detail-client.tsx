  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
      <Card variant="gradient">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-xl"><Users className="h-5 w-5 text-accent-primary" />Member progress</CardTitle>
          <CardDescription>Shared accountability without chat noise — chapter completion, quiz strength, and streak momentum.</CardDescription>
        </CardHeader>
        <CardBody className="space-y-3">
          <div className="overflow-hidden rounded-2xl border border-border-default">
            <table className="min-w-full divide-y divide-border-default text-sm">
              <thead className="bg-bg-subtle/80 text-left text-xs uppercase tracking-[0.16em] text-text-secondary"><tr><th className="px-4 py-3 font-medium">Member</th><th className="px-4 py-3 font-medium">Chapter completion</th><th className="px-4 py-3 font-medium">Best quiz</th><th className="px-4 py-3 font-medium">Streak</th></tr></thead>
              <tbody className="divide-y divide-border-default bg-bg-surface/80">{payload.members.map((member) => <tr key={member.userId}><td className="px-4 py-3 font-medium text-text-primary">{member.name}</td><td className="px-4 py-3 text-text-secondary">{member.chapterCompletionPercent}%</td><td className="px-4 py-3 text-text-secondary">{member.bestQuizScorePercent}%</td><td className="px-4 py-3 text-text-secondary">{member.streakDays} day{member.streakDays === 1 ? "" : "s"}</td></tr>)}</tbody>
            </table>
          </div>
        </CardBody>
      </Card>
