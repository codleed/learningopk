...
export function AdminAnalyticsSubjectTable({ rows }: AdminAnalyticsSubjectTableProps) {
  if (rows.length === 0) {
    return <p className="text-sm text-text-secondary">No subject analytics data is available for this window.</p>;
  }

  return (
    <div className="overflow-x-auto">
      <table aria-label="Subject performance" className="min-w-full divide-y divide-border-default text-sm">
        ...
        <tbody className="divide-y divide-border-default">
          {rows.map((row) => (
            <tr key={row.subjectId}>
              ...
              <td className="px-3 py-2 text-text-primary/90">{row.averageScorePercent.toFixed(1)}%</td>
