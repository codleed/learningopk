...
import {
  deleteModerationReply,
  deleteModerationThread,
  type AdminModerationFlag,
} from "@/lib/admin-api";
...
export function ModerationQueueTable({ rows, onResolved }: ModerationQueueTableProps) {
  if (rows.length === 0) {
    return (
      <p className="text-sm text-muted-foreground">No moderation flags match current filters.</p>
    );
  }
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        try {
                          ...
                          await fetch(
                            `${process.env.NEXT_PUBLIC_BACKEND_URL ?? "http://localhost:3001"}/api/admin/moderation/flags/${flag.id}/resolve`,
                            {
                              method: "POST",
