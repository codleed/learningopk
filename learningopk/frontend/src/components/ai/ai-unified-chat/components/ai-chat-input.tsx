...
export function AIChatInput({
  ...
  const isLimitReached = rateLimitRemaining !== null && rateLimitRemaining !== undefined && rateLimitRemaining <= 0;
  const isLimitLow = rateLimitRemaining !== null && rateLimitRemaining !== undefined && rateLimitRemaining > 0 && rateLimitRemaining <= 5;
  ...
  return (
    <div
      className={cn('px-4 py-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] border-t border-border-default/50 bg-bg-surface/50', className)}
    >
        ...
        {stoppedStatus ? (
          ...
          <>
            ...
            <p className="text-[11px] text-text-secondary/60">
              {isStreaming ? 'AI is generating...' : isSending ? 'AI is thinking...' : 'Shift + Enter for new line'}
            </p>
          </>
        )}
      </div>
