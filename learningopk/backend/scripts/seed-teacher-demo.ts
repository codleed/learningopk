...
async function main() {
  ...
  console.log(
    `✅ Classrooms created: "${classroom9.name}" (invite: ${classroom9.inviteCode}), "${classroom10.name}" (invite: ${classroom10.inviteCode})`
  );
  ...
  const studentData = [
    ...
    {
      name: "Usman Javed",
      email: `usman-ts-${ts}@student.pk`,
      class: "10",
      classroomId: classroom10.id,
      xp: 3200,
      level: 15,
    },
    {
      name: "Ayesha Noor",
      email: `ayesha-ts-${ts}@student.pk`,
      class: "10",
      classroomId: classroom10.id,
      xp: 2500,
      level: 12,
  ...
  for (const s of studentData) {
    ...
    console.log(
      `✅ Student: ${s.name} (Level ${s.level}, ${s.xp} XP) → Classroom ${s.classroomId}`
    );
  }
  ...
  console.log(
    `📚 Found ${chapterRows.length} chapters, ${quizRows.length} quizzes for progress seeding`
  );
  ...
  for (const studentId of studentIds) {
    for (let ci = 0; ci < Math.min(4, chapterRows.length); ci++) {
      ...
      const quizScore = Math.min(
        100,
        Math.max(15, bases[scoreIdx % bases.length]! + Math.floor(Math.random() * 10 - 5))
      );
  ...
  const announcements = [
    {
      classroomId: classroom9.id,
      content: "Welcome to 9th Grade Math! Please review the Algebra Basics assignment.",
      pinned: true,
    },
    {
      classroomId: classroom9.id,
      content: "Reminder: Geometry quiz due this Friday. Office hours Thursday 3-4 PM.",
      pinned: false,
    },
    {
      classroomId: classroom9.id,
      content: "Great work on Linear Equations! Average class score: 78%. Keep it up!",
      pinned: false,
    },
    {
      classroomId: classroom10.id,
      content: "Welcome to 10th Grade Math! Mock exam scheduled for end of month.",
      pinned: true,
    },
    {
      classroomId: classroom10.id,
      content: "Trigonometry scores posted. See me if you scored below 60% for extra practice.",
      pinned: false,
    },
  ];
