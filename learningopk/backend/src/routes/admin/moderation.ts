...
moderationAdminRouter.post(
  "/moderation/threads/:threadId/edit",
  requireSession,
  async (req, res) => {
    ...
    if (!parsedParams.success) {
      res
        .status(400)
        .json({ error: "Invalid thread identifier", details: parsedParams.error.flatten() });
      return;
    }
...
moderationAdminRouter.post(
  "/moderation/threads/:threadId/delete",
  requireSession,
  async (req, res) => {
    const parsedParams = threadParamsSchema.safeParse(req.params);
    if (!parsedParams.success) {
...
moderationAdminRouter.post(
  "/moderation/replies/:replyId/edit",
  requireSession,
  async (req, res) => {
    ...
    if (!parsedParams.success) {
      res
        .status(400)
        .json({ error: "Invalid reply identifier", details: parsedParams.error.flatten() });
      return;
    }
...
moderationAdminRouter.post(
  "/moderation/replies/:replyId/delete",
  requireSession,
  async (req, res) => {
    const parsedParams = replyParamsSchema.safeParse(req.params);
    if (!parsedParams.success) {
...
moderationAdminRouter.post("/moderation/users/:id/warn", requireSession, async (req, res) => {
  ...
  if (!parsedParams.success) {
    res
      .status(400)
      .json({ error: "Invalid user identifier", details: parsedParams.error.flatten() });
    return;
  }
...
moderationAdminRouter.post("/moderation/users/:id/temp-ban", requireSession, async (req, res) => {
  ...
  if (!parsedBody.success) {
    res
      .status(400)
      .json({ error: "Invalid temp-ban payload", details: parsedBody.error.flatten() });
    return;
  }
  const authedReq = req as AuthenticatedRequest;
  if (!(await requireStaffRole(authedReq, res))) return;
...
moderationAdminRouter.get("/moderation/user-history/:id", requireSession, async (req, res) => {
  ...
  const [flags, warnings] = await Promise.all([
    db
      ...
      .where(or(eq(forumThreads.userId, userId), eq(forumReplies.userId, userId)))
