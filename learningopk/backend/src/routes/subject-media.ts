...
subjectMediaRouter.post("/subjects/:subjectId/cover-image", requireSession, uploadSubjectCoverImage, async (req, res) => {
  const parsedParams = subjectIdParamsSchema.safeParse(req.params);
  if (!parsedParams.success) {
    res.status(400).json(errorResponse("Invalid subject identifier.", "VALIDATION_ERROR", parsedParams.error.flatten()));
    return;
  }
  ...
  try {
    ...
    res.status(500).json(errorResponse("Failed to upload subject cover image.", "INTERNAL_ERROR"));
  }
});
