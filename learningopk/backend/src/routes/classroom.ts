...
classroomRouter.post("/join", requireSession, async (req, res) => {
  ...
  if (existingEnrollment) {
    res
      ...
      .json(
        errorResponse(
          "You are already enrolled in a classroom. Leave it before joining another.",
          "ALREADY_ENROLLED"
        )
      );
    return;
...
classroomRouter.get("/:id/assignments", requireSession, async (req, res) => {
  ...
  const submissions = await classroomRepository.getSubmissionsForStudentInClassroom(
    classroomId,
    userId
  );
