...
test("buildPublicObjectUrl includes bucket in path by default (MinIO / path-style S3)", () => {
  ...
  assert.equal(url, "http://localhost:9000/learningo-media/profile-images/user_123/asset_001.png");
});
...
test("buildPublicObjectUrl omits bucket in path for S3 virtual-hosted style", () => {
  ...
  assert.equal(url, "http://localhost:9000/profile-images/user_123/asset_001.png");
});
