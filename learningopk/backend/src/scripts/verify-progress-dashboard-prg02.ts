...
const run = async (): Promise<void> => {
  ...
  if (unauthDashboardResponse.status !== 401) {
    throw new Error(
      `Expected unauthenticated dashboard request to return 401, got ${unauthDashboardResponse.status}`
    );
  }
  ...
  if (chapterVisitResponse.status !== 200) {
    throw new Error(
      `Expected chapter visit event to return 200, got ${chapterVisitResponse.status}`
    );
  }
  ...
  if (exerciseViewResponse.status !== 200) {
    throw new Error(
      `Expected exercise view event to return 200, got ${exerciseViewResponse.status}`
    );
  }
  ...
  if (userOneDashboardResponse.status !== 200) {
    throw new Error(
      `Expected user one dashboard request to return 200, got ${userOneDashboardResponse.status}`
    );
  }
  ...
  if (userTwoDashboardResponse.status !== 200) {
    throw new Error(
      `Expected user two dashboard request to return 200, got ${userTwoDashboardResponse.status}`
    );
  }
  ...
  if (userOneDashboard.studentName !== "Dashboard User One") {
    throw new Error(
      `Expected dashboard studentName for user one, got ${userOneDashboard.studentName}`
    );
  }
  if (userTwoDashboard.studentName !== "Dashboard User Two") {
    throw new Error(
      `Expected dashboard studentName for user two, got ${userTwoDashboard.studentName}`
    );
  }

  const userOneActiveSubject = userOneDashboard.subjects.find(
    (subject) => subject.chaptersVisitedPercent > 0
  );
  ...
  if (userOneDashboard.weeklyActivity.length !== 7) {
    throw new Error(
      `Expected weeklyActivity length=7 for user one, got ${userOneDashboard.weeklyActivity.length}`
    );
  }

  if (userOneDashboard.streakDays < 1) {
    throw new Error(
      `Expected streakDays >= 1 for active user one, got ${userOneDashboard.streakDays}`
    );
  }

  const userTwoHasVisited = userTwoDashboard.subjects.some(
    (subject) => subject.chaptersVisitedPercent > 0
  );
  ...
  if (userTwoDashboard.recentActivity.length !== 0) {
    throw new Error(
      `Expected no recent activity for user two, got ${userTwoDashboard.recentActivity.length}`
    );
  }

  if (userTwoDashboard.streakDays !== 0) {
    throw new Error(
      `Expected streakDays=0 for inactive user two, got ${userTwoDashboard.streakDays}`
    );
  }

  const userTwoActiveHeatmapDays = userTwoDashboard.weeklyActivity.filter(
    (entry) => entry.active
  ).length;
