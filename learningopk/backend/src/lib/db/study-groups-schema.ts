...
export const studyGroupActivityTypeEnum = pgEnum("study_group_activity_type", ["chapter_completed", "quiz_score_beaten"]);
...
export const studyGroupActivities = pgTable(
  ...
  (table) => [
    ...
    index("study_group_activities_recipient_created_idx").on(table.recipientUserId, desc(table.createdAt))
  ]
);
