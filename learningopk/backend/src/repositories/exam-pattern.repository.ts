...
export class ExamPatternRepository {
  async refreshAnalysisForSubject(subjectId: number, boardId: number) {
    await db
      .delete(examAnalysis)
      .where(and(eq(examAnalysis.subjectId, subjectId), eq(examAnalysis.boardId, boardId)));
    ...
    return withOptionalDbFallback(
      ...
      async () => {
        ...
        const latestYearRows = await db
          ...
          .from(mockExams)
          .where(
            and(eq(mockExams.subjectId, context.subjectId), eq(mockExams.boardId, context.boardId))
          );

        const latestYear = latestYearRows[0]?.latestYear ?? null;
        const startYear = latestYear === null ? null : latestYear - (ANALYSIS_WINDOW_YEARS - 1);
        const totalYears =
          latestYear === null || startYear === null ? 0 : latestYear - startYear + 1;
        ...
        const chapterRows = await db
          ...
          .orderBy(
            desc(sql`coalesce(${examAnalysis.occurrenceCount}, 0)`),
            desc(sql`coalesce(${examAnalysis.avgMarks}, 0)`),
            asc(chapters.chapterNumber)
          );

        const trendRows =
          latestYear === null || startYear === null
            ? []
            : await db
                .select({
                  chapterId: chapters.id,
        ...
        const weightedChapters = chapterRows.map((row) => {
          ...
          return {
            ...
            trend: years.map((year) => ({
              year,
              marks: trendMap.get(row.chapterId)?.get(year) ?? 0,
            })),
          };
        });

        const focusChapters = weightedChapters
          .slice(0, 3)
          .filter((chapter) => chapter.weightagePercentage > 0);
  ...
  async findChapterPatternByRoute(params: ChapterRoute) {
    ...
    const chapterPattern =
      subjectPatterns.chapters.find((chapter) => chapter.slug === params.chapter) ?? null;
    if (!chapterPattern) {
      return null;
    }
